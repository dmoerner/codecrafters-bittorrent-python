...
def request_block(s, piece_index, block_index, length):
    ...
    length = length
    payload = struct.pack('>I', index) + struct.pack('>I', begin) + struct.pack('>I', length)
...
def main():
    ...
    if command == "decode":
        ...
        if len(sys.argv) != 6:
            raise NotImplementedError(f"Usage: {sys.argv[0]} download_piece -o output filename piececount")
